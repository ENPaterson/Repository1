...
ARIMA  %>% 
  ...
  ggplot(aes(x=as.factor(year), y=male_SH_admitted_n))+
...
ARIMA %>% 
  ggplot(aes(x=month, y=male_SH_admitted_n))+
  geom_line()+
  labs(
    y = "Individuals Presenting (n)"
  )
...
ARIMA  %>% 
  ...
  ggplot(aes(x=c_month, y=male_SH_admitted_n))+
  geom_line()+
  facet_wrap(~year)+
  labs(
    x = "Month",
...
ARIMA_2012_2019 <- ARIMA$male_SH_admitted_n[1:95]      
...
ARIMA_2020 <- ARIMA$male_SH_admitted_n[96:102]   
