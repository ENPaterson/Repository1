...
ARIMA  %>% 
  ...
  ggplot(aes(x=as.factor(year), y=total_admitted_n100k))+
  geom_bar(stat="identity")+
  facet_wrap(~c_month)+
  labs(
    x = "Month",
    y = "Individuals Presenting (n/100k)"
  )+
  theme(axis.text.x = element_text(angle = 90))

#Visualise monthly variation with a line graph
ARIMA %>% 
  ggplot(aes(x=month, y=total_admitted_n100k))+
  geom_line()+
  labs(
...
ARIMA  %>% 
  ...
  ggplot(aes(x=c_month, y=total_admitted_n100k))+
  geom_line()+
  facet_wrap(~year)+
  labs(
    x = "Month",
...
ARIMA_2012_2019 <- ARIMA$total_admitted_n100k[1:95]      
...
ARIMA_2020 <- ARIMA$total_admitted_n100k[96:102]   
...
autoplot(ARIMA_2012_2019_ts) +  ylab("Individuals Presenting (n/100k)")
...
autoplot(diff_ARIMA_2012_2019) +
  ylab("Individuals Presenting (n/100k)")

##Check for seasonality in stationary data set (i.e. the differenced data)
ggseasonplot(diff_ARIMA_2012_2019) + ylab("Individuals Presenting (n/100k)")

#Alternative view of seasonal variation
ggsubseriesplot(diff_ARIMA_2012_2019) +  ylab("Individuals Presenting (n/100k)")
