...
ARIMA  %>% 
  ...
  ggplot(aes(x=as.factor(year), y=urban_admitted_n))+
...
ARIMA %>% 
  ggplot(aes(x=month, y=urban_admitted_n))+
  geom_line()+
  labs(
    y = "Presentations (n)"
  )
...
ARIMA  %>% 
  ...
  ggplot(aes(x=c_month, y=urban_admitted_n))+
  geom_line()+
  facet_wrap(~year)+
  labs(
    x = "Month",
...
ARIMA_2012_2019 <- ARIMA$urban_admitted_n[1:95]      
...
ARIMA_2020 <- ARIMA$urban_admitted_n[96:102]   
