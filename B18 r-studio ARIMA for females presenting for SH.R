...
ARIMA  %>% 
  ...
  ggplot(aes(x=as.factor(year), y=female_SH_admitted_n))+
...
ARIMA %>% 
  ggplot(aes(x=month, y=female_SH_admitted_n))+
  geom_line()+
  labs(
    y = "Individuals Presenting (n)"
  )
...
ARIMA  %>% 
  ...
  ggplot(aes(x=c_month, y=female_SH_admitted_n))+
  geom_line()+
  facet_wrap(~year)+
  labs(
    x = "Month",
...
ARIMA_2012_2019 <- ARIMA$female_SH_admitted_n[1:95]      
...
ARIMA_2020 <- ARIMA$female_SH_admitted_n[96:102]   
