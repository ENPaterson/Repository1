...
ARIMA  %>% 
  ...
  ggplot(aes(x=as.factor(year), y=admitted_n_65))+
...
ARIMA %>% 
  ggplot(aes(x=month, y=admitted_n_65))+
  geom_line()+
  labs(
    y = "Individuals Presenting (n)"
  )
...
ARIMA  %>% 
  ...
  ggplot(aes(x=c_month, y=admitted_n_65))+
  geom_line()+
  facet_wrap(~year)+
  labs(
    x = "Month",
...
ARIMA_2012_2019 <- ARIMA$admitted_n_65[1:95]      
...
ARIMA_2020 <- ARIMA$admitted_n_65[96:102]   
