...
ARIMA  %>% 
  ...
  ggplot(aes(x=as.factor(year), y=total_admissions_n100k))+
  geom_bar(stat="identity")+
  facet_wrap(~c_month)+
  labs(
    x = "Month",
    y = "Presentations (n/100k)"
  )+
  theme(axis.text.x = element_text(angle = 90))

#Visualise monthly variation with a line graph
ARIMA %>% 
  ggplot(aes(x=month, y=total_admissions_n100k))+
  geom_line()+
  labs(
    y = "Presentations (n/100k)"
  )
...
ARIMA  %>% 
  ...
  ggplot(aes(x=c_month, y=total_admissions_n100k))+
  geom_line()+
  facet_wrap(~year)+
  labs(
    x = "Month",
...
ARIMA_2012_2019 <- ARIMA$total_admissions_n100k[1:93]      

# POSTCOVID Jan 2020 to Sep 2020
# sh_month 4 - 96 (from rows 94-102)
ARIMA_2020 <- ARIMA$total_admissions_n100k[94:102]   

#Datasets created in STATA
#Create time series for precovid data
#This syntax only works after the preceeding steps have been performed.
...
autoplot(ARIMA_2012_2019_ts) +   ylab("Presentations (n/100k)")
...
autoplot(diff_ARIMA_2012_2019) +   ylab("Presentations (n/100k)")

##Check for seasonality in stationary data set (i.e. the differenced data)
ggseasonplot(diff_ARIMA_2012_2019) +  ylab("Presentations (n/100k)")

#Alternative view of seasonal variation
ggsubseriesplot(diff_ARIMA_2012_2019) +  ylab("Presentations (n/100k)")
