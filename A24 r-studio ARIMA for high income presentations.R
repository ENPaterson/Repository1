...
ARIMA  %>% 
  ...
  ggplot(aes(x=as.factor(year), y=high_income_admissions_n))+
...
ARIMA %>% 
  ggplot(aes(x=month, y=high_income_admissions_n))+
  geom_line()+
  labs(
    y = "Presentations (n)"
  )
...
ARIMA  %>% 
  ...
  ggplot(aes(x=c_month, y=high_income_admissions_n))+
  geom_line()+
  facet_wrap(~year)+
  labs(
    x = "Month",
...
ARIMA_2012_2019 <- ARIMA$high_income_admissions_n[1:93]      

# POSTCOVID Jan 2020 to Sep 2020
# sh_month 4 - 96 (from rows 94-102)
ARIMA_2020 <- ARIMA$high_income_admissions_n[94:102]   
