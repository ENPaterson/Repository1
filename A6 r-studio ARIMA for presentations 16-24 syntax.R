...
ARIMA  %>% 
  ...
  ggplot(aes(x=as.factor(year), y=admissions_n_1624))+
...
ARIMA %>% 
  ggplot(aes(x=month, y=admissions_n_1624))+
  geom_line()+
  labs(
    y = "Presentations (n)"
  )
...
ARIMA  %>% 
  ...
  ggplot(aes(x=c_month, y=admissions_n_1624))+
  geom_line()+
  facet_wrap(~year)+
  labs(
    x = "Month",
...
ARIMA_2012_2019 <- ARIMA$admissions_n_1624[1:93]      

# POSTCOVID Jan 2020 to Sep 2020
# sh_month 4 - 96 (from rows 94-102)
ARIMA_2020 <- ARIMA$admissions_n_1624[94:102]   
